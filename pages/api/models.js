...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    // skip permission check for signup user creation
    try {
      let modelName, modelNameLower, fields;

      if (req.body.modelType && req.body.data) {
        // handle user model creation from signup
        modelName = req.body.modelType;
        ...
        fields = Object.entries(userData).map(([key, value]) => {
          let fieldType, datatype, enumValues;
          if (modelNameLower === "user" && userFieldTypes[key]) {
            fieldType = userFieldTypes[key].fieldType;
            datatype = userFieldTypes[key].datatype;
            enumValues = userFieldTypes[key].enumValues;
          } else {
      ...
      const modelPath = path.join(
        process.cwd(),
        "models",
        `${capitalizeFirstLetter(modelNameLower)}.js`
      );
      const apiPath = path.join(
        process.cwd(),
        "pages",
      ...
      fields.forEach((field) => {
        if (field.refModel) addReverseRelation(modelNameLower, field.refModel);
      });
      ...
      try {
        ...
        console.error(
          `Error loading model ${modelName} dynamically:`,
          dynamicErr
        );
      }

      return res.status(201).json({
        message: "Model, API, Pages, and Component generated successfully",
      });
    } catch (err) {
      console.error("Error creating model:", err);
      return res
  ...
  if (req.method === "PUT") {
    ...
    try {
      ...
      try {
        ...
        console.log(`model ${modelNameLower} dynamically loaded successfully`);
      } catch (dynamicErr) {
        console.error(
          `Error loading model ${modelNameLower} dynamically:`,
          dynamicErr
        );
      }
      return res
